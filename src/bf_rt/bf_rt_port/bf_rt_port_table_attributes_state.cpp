...
int bfRtPortStatusChgInternalCb(bf_dev_id_t dev_id,
  ...
  auto attributes_state = static_cast<BfRtStateTableAttributesPort *>(cookie);
  ...
  // cast to BfRtPortCfgTableKey
  auto matchKey = dynamic_cast<BfRtPortCfgTableKey *>(newKey.get());
  if (matchKey == nullptr) {
    LOG_ERROR(
...
void BfRtStateTableAttributesPort::stateTableAttributesPortSet(
    bool enabled,
    BfRtPortStatusNotifCb callback_fn,
    bf_rt_port_status_chg_cb callback_fn_c,
...
void BfRtStateTableAttributesPort::stateTableAttributesPortReset() {
  enabled_ = false;
  callback_ = nullptr;
  callback_c_ = nullptr;
  table_ = nullptr;
...
std::tuple<bool,
           ...
           const BfRtTableObj *,
           void *>
BfRtStateTableAttributesPort::stateTableAttributesPortGet() {
  std::lock_guard<std::mutex> lock(state_lock);
