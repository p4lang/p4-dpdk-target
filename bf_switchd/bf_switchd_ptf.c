				  ...
				  const int operation)
{
	...
	const bf_rt_id_t field_id = 1;
	const bf_rt_id_t action_id = 18638031;
	const bf_rt_id_t data_field_id = 1;

	uint32_t port_nw = (uint32_t)htonl((uint32_t)port);
	...
	switch(operation)
	{
		...
		//this case will run entry addition config executed
		case ADD_RULE:
			// allocate data hdl
